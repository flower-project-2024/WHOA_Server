...
import com.whoa.whoaserver.domain.bouquet.dto.response.BouquetInfoDetailResponse;
import com.whoa.whoaserver.domain.bouquet.service.BouquetCustomizingService;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.*;

import com.whoa.whoaserver.domain.bouquet.dto.request.BouquetCustomizingRequest;
import com.whoa.whoaserver.domain.bouquet.dto.response.BouquetCustomizingResponse;
...
@Tag(name = "1차 MVP Bouquet Customizing", description = "Header에 MEMBER_ID(key), 디바이스 등록 이후 반환 받은 id(value)로 요청해주세요.")
@RestController
...
public class BouquetCustomizingController {
	...
	public ResponseEntity<BouquetCustomizingResponse> registerBouquet(
		@DeviceUser UserContext userContext,
		@Valid @RequestBody BouquetCustomizingRequest request
	) {
	...
	public ResponseEntity<BouquetCustomizingResponse> updateBouquet(
		@DeviceUser UserContext userContext,
		@Valid @RequestBody BouquetCustomizingRequest request,
	...
	public ResponseEntity<Void> deleteBouquet(
		@DeviceUser UserContext userContext,
		@PathVariable("bouquetId") final Long bouquetId
	) {
		Long memberId = userContext.id();
	...
	public ResponseEntity<BouquetInfoDetailResponse> getBouquetDetails(
		@DeviceUser UserContext userContext,
		@PathVariable("bouquetId") final Long bouquetId
	) {
